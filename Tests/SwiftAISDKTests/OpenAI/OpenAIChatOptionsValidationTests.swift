...
struct OpenAIChatOptionsValidationTests {
    @Test("Reject logitBias non-numeric keys")
    func rejectLogitBiasNonNumericKey() async throws {
        actor CallCounter {
            var count = 0
        ...
        do {
            _ = try await model.doGenerate(
                options: LanguageModelV3CallOptions(
                    ...
                    providerOptions: ["openai": [
                        "logitBias": JSONValue.object(["not-a-number": .number(1)])
                    ]]
                )
            )
            ...
            #expect(
                String(describing: schemaError.issues).contains("logitBias keys must be numbers"),
                "Expected logitBias key validation issue"
            )
        }
