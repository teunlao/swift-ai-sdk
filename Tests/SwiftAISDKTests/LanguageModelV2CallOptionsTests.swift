...
@Suite("LanguageModelV2 CallOptions")
struct LanguageModelV2CallOptionsTests {
    ...
    func minimal() throws {
        ...
        let opts = LanguageModelV2CallOptions(prompt: prompt)
    ...
    func full() throws {
        ...
        let toolCallPart = LanguageModelV2MessagePart.toolCall(.init(
            toolCallId: "c1",
            toolName: "search",
        ...
        let functionTool = LanguageModelV2Tool.function(.init(name: "search", inputSchema: ["type": .string("object")], description: "find"))
        let providerTool = LanguageModelV2Tool.providerDefined(.init(id: "code-exec", name: "Code Execution", args: [:]))

        let schema: JSONValue = ["type": .string("object")]
        let opts = LanguageModelV2CallOptions(
            prompt: prompt,
            maxOutputTokens: 512,
            temperature: 0.2,
            stopSequences: ["\n\n"],
