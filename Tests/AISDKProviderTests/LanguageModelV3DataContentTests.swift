...
func v3_dataContent_encode_base64_plainString() throws {
    let dc = LanguageModelV3DataContent.base64("QUJD")
...
func v3_dataContent_encode_url_plainString() throws {
    let url = URL(string: "https://example.com/a.png")!
    let dc = LanguageModelV3DataContent.url(url)
    let data = try JSONEncoder().encode(dc)

    // Decode back to verify it's a plain string (not wrapped object)
    let decoded = try JSONDecoder().decode(LanguageModelV3DataContent.self, from: data)
    #expect(decoded == dc)
}

@Test("DataContent encode: Data uses base64 string")
func v3_dataContent_encode_data_base64() throws {
    let bytes = Data([0x00, 0x01, 0x02])
    let dc = LanguageModelV3DataContent.data(bytes)
    let data = try JSONEncoder().encode(dc)
    let s = String(data: data, encoding: .utf8)!
    #expect(s == "\"AAEC\"")
}
...
func v3_dataContent_decode_legacy_base64_wrapper() throws {
    let json = "{\"type\":\"base64\",\"data\":\"QUJD\"}".data(using: .utf8)!
    let dc = try JSONDecoder().decode(LanguageModelV3DataContent.self, from: json)
    #expect(dc == .base64("QUJD"))
}

@Test("DataContent decode: legacy wrapped url object")
func v3_dataContent_decode_legacy_url_wrapper() throws {
