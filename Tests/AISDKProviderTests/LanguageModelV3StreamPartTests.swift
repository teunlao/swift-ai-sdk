...
func v3_streamPart_text_reasoning_roundTrip() throws {
    let pm: SharedV3ProviderMetadata = [
        "provider": [
            "k": .string("v"),
            "n": .number(1)
        ]
    ...
    for p in parts {
        ...
        let back = try dec.decode(LanguageModelV3StreamPart.self, from: data)
        #expect(back == p)
    }
}

@Test("StreamPart round-trip: tool-input start/delta/end + tool-call/result")
func v3_streamPart_toolInput_and_callResult_roundTrip() throws {
    let pm: SharedV3ProviderMetadata = [
...
func v3_streamPart_responseMetadata_iso8601_decode() throws {
    let json = """
    {"type":"response-metadata","id":"r1","modelId":"gpt-xyz","timestamp":"2025-10-12T00:00:00Z"}
    """.data(using: .utf8)!
    let dec = JSONDecoder()
    dec.dateDecodingStrategy = .iso8601
    let part = try dec.decode(LanguageModelV3StreamPart.self, from: json)
...
func v3_streamPart_finish_usage_roundTrip() throws {
    ...
    let back = try JSONDecoder().decode(LanguageModelV3StreamPart.self, from: data)
    #expect(back == part)
}

@Test("StreamPart round-trip: raw + error arbitrary JSON payloads")
func v3_streamPart_raw_error_roundTrip() throws {
    let raw: LanguageModelV3StreamPart = .raw(rawValue: [
        "arr": .array([.number(1), .string("x")]),
        "obj": .object(["a": .bool(true)])
    ])
    let err: LanguageModelV3StreamPart = .error(error: .object([
