...
## Автоматизированный цикл валидации
- Исполнитель работает под системным промтом и обязан вести `.orchestrator/flow/<agent-id>.json` (статус, итерация, ссылки на запрос/отчёт, блокеры).
- После завершения итерации нужно сформировать Markdown-файл запроса в `.orchestrator/requests/…` и обновить flow-файл, установив `status = "ready_for_validation"` и `request.ready = true`.
- MCP-сервер отслеживает эти файлы и автоматически создаёт валидационную сессию, запускает валидатора в том же worktree и связывает его с задачей.
- Если отчёт валидатора содержит `result = "rejected"`, автоматизация сама отправит follow-up промт через `continue_agent`, переведёт исполнителя в `running` и будет ждать следующую итерацию.
- В случае, когда работа застопорилась, выставляй `status = "needs_input"` и добавляй подробные `blockers` — автоматизация остановится до ручного вмешательства.
