...
public enum TextStreamPart: Sendable {
    ...
    /// Provider-specific custom content emitted by a V4 model.
    case custom(kind: String, providerMetadata: ProviderMetadata?)

    /// A file that was generated by the model.
    case file(GeneratedFile)

    /// A reasoning file that was generated by the model.
    case reasoningFile(file: GeneratedFile, providerMetadata: ProviderMetadata?)
...
extension TextStreamPart: Equatable {
    public static func == (lhs: TextStreamPart, rhs: TextStreamPart) -> Bool {
        ...
        case let (.custom(lKind, lMeta), .custom(rKind, rMeta)):
            return lKind == rKind && lMeta == rMeta
        case (.file, .file):
            // GeneratedFile is a protocol, cannot compare directly
            return true
        case let (.reasoningFile(_, lMeta), .reasoningFile(_, rMeta)):
            // GeneratedFile is a protocol, cannot compare directly
            return lMeta == rMeta
...
extension TextStreamPart: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        // Content events
        case "source":
            let source = try Source(from: decoder)
            self = .source(source)

        case "custom":
            let kind = try container.decode(String.self, forKey: .kind)
            let metadata = try container.decodeIfPresent(ProviderMetadata.self, forKey: .providerMetadata)
            self = .custom(kind: kind, providerMetadata: metadata)
        ...
        case "reasoning-file":
            ...
            let file = DefaultGeneratedFileWithType(base64: base64, mediaType: mediaType)
            let metadata = try container.decodeIfPresent(ProviderMetadata.self, forKey: .providerMetadata)
            self = .reasoningFile(file: file, providerMetadata: metadata)
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .reasoningFile(let file, let providerMetadata):
            try container.encode("reasoning-file", forKey: .type)
            try container.encodeIfPresent(providerMetadata, forKey: .providerMetadata)
            var fileContainer = encoder.container(keyedBy: FileCodingKeys.self)
            try fileContainer.encode(file.base64, forKey: .base64)
            try fileContainer.encode(file.mediaType, forKey: .mediaType)
...
extension TextStreamPart: CustomStringConvertible {
    public var description: String {
        ...
        case let .custom(kind, _):
            return "custom(kind: \(kind))"
        case .file(let file):
            return "file(mediaType: \(file.mediaType))"
        case .reasoningFile(let file, _):
            return "reasoningFile(mediaType: \(file.mediaType))"
        case .toolCall:
            return "toolCall(...)"
        case .toolResult:
            return "toolResult(...)"
