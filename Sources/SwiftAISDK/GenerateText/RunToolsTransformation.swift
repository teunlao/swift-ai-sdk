...
private func makeInvalidToolCallError(from toolCall: TypedToolCall) -> TypedToolError {
    let message = errorMessage(from: toolCall.error)
    let error = NSError(
        domain: "RunToolsTransformation",
        code: -1,
...
private func errorMessage(from error: (any Error)?) -> String {
    ...
    return AISDKProvider.getErrorMessage(error)
}
