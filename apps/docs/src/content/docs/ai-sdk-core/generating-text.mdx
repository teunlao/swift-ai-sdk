...
let cityAttractions = dynamicTool(
  ...
  execute: { input, _ in
    return .object(["attractions": .array([.string("attraction1"), .string("attraction2"), .string("attraction3")])])
  }
)

let result = try streamText(
  model: openai("gpt-4.1"),
  tools: ["cityAttractions": cityAttractions],
