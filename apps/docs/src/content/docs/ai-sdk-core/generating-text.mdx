...
### Request objects (base + override) (v0.5.5+)

Swift requires passing function arguments in the declared order. When calls become large (tools, settings, provider options, callbacks), it can be hard to “override just one field” like in TypeScript.

`GenerateTextRequest` lets you build a reusable base request and override only what changes per call:

```swift
import SwiftAISDK
import OpenAIProvider

let base = GenerateTextRequest(
  model: openai("gpt-5"),
  toolChoice: .auto,
...
var r1 = base
r1.prompt = "A"
let a = try await generateText(r1)

var r2 = base
r2.prompt = "B"
let b = try await generateText(r2)

print(a.text)
print(b.text)
```

The existing `generateText(...)` overloads remain unchanged; this is a convenience wrapper around them.
...
- `result.experimentalOutput`: The generated structured output (configured via `experimentalOutput:`).
- `result.experimentalOutputIfSpecified`: Convenience helper that returns `nil` when no output parser was configured (v0.5.4+).

### Inspecting the full result

Need to debug the entire payload (same as `JSON.stringify` in TypeScript)? Call `jsonString()` or `jsonValue()` directly on the result.
...
let fullJSON = try result.jsonString()
print(fullJSON)
```

Both helpers mirror the upstream object shape, so you can log or persist the response without manually traversing the structure.
...
let _ = try await generateText(
  ...
  onFinish: { event in
    let followUp = try await generateText(
      model: openai("gpt-4o"),
      messages: event.response.messages + [.user("Summarize in 2 sentences.")]
    )
...
```
> Tip
> `response.messages` returns `[ResponseMessage]`. Combine it with existing history via `messages = messages + response.messages` or call `response.messages.asModelMessages()` to get `[ModelMessage]`.
...
`StreamTextRequest` works the same way. Build a reusable base request, then set `prompt` (or `messages`) right before streaming:

```swift
import SwiftAISDK
import OpenAIProvider

let base = StreamTextRequest(
  model: openai("gpt-5"),
  providerOptions: ["openai": ["reasoningEffort": .string("low")]],
  settings: CallSettings(maxRetries: 2)
)
...
var req = base
req.prompt = "Invent a new holiday and describe its traditions."
req.onError = { error in
  print("Stream error: \(error)")
}

let result = try streamText(req)
...
let _ = try streamText(
  ...
  onFinish: { finalStep, steps, totalUsage, finishReason in
    let followUp = try await generateText(
      model: openai("gpt-4o"),
      messages: finalStep.response.messages + [.user("Summarize the plan in 2 sentences.")]
    )
    Logger.info(followUp.text)
  }
)
