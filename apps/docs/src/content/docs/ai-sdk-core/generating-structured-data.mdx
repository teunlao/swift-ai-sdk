...
You can describe the shape using [JSON Schema](/docs/reference/ai-sdk-core/json-schema),
and the AI model will generate data that conforms to that structure.

> Note
> In the Swift port, prefer `FlexibleSchema(jsonSchema(...))` or `Schema.codable(...)` for Decodable types.
    ...
    "properties": .object([
      "recipe": .object([
        ...
        "properties": .object([
          ...
          "steps": .object(["type": .string("array"), "items": .object(["type": .string("string")])])
        ]),
...
```swift
let heroSchema = FlexibleSchema(jsonSchema(
  .object([
    "type": .string("object"),
    "properties": .object([
      "name": .object(["type": .string("string")]),
      "class": .object(["type": .string("string"), "description": .string("Character class, e.g. warrior, mage, or thief.")]),
      "description": .object(["type": .string("string")])
    ]),
    "required": .array([.string("name"), .string("class"), .string("description")])
  ])
))
...
do {
  _ = try await generateObject(model: openai("gpt-5"), schema: recipeSchema, prompt: "...")
...
```swift
let personSchema = FlexibleSchema(jsonSchema(
  .object([
    "type": .string("object"),
    "properties": .object([
      "name": .object(["type": .string("string")]),
      "age": .object(["type": .string("number"), "nullable": .bool(true), "description": .string("Age of the person.")]),
      "contact": .object(["type": .string("object"), "properties": .object([
        "type": .object(["type": .string("string")]),
        "value": .object(["type": .string("string")])
      ])]),
      "occupation": .object(["type": .string("object"), "properties": .object([
        "type": .object(["type": .string("string")]),
        "company": .object(["type": .string("string")]),
        "position": .object(["type": .string("string")])
      ])])
    ])
  ])
))
...
let gt = try await generateText(
  model: openai("gpt-5"),
  experimentalOutput: Output.object(schema: personSchema),
