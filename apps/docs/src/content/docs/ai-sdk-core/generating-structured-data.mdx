...
> Note
> In the Swift port, the most ergonomic option is `generateObject(schema: MyCodableType.self)`. For advanced scenarios you can still fall back to `FlexibleSchema<MyType>.jsonSchema(...)` or `Schema.codable(...)`.
...
### Typed partial decoding (v0.5.3+)

By default, `partialObjectStream` yields `[String: JSONValue]`. If you'd like a typed partial instead, use `decodedPartialObjectStream(as:)`:

```swift
struct RecipePartial: Codable, Sendable {
  let name: String?
  let ingredients: [Ingredient]?
  let steps: [String]?
}

for try await partial in stream.decodedPartialObjectStream(as: RecipePartial.self) {
  print(partial.name ?? "")
}
```

This decoding is best-effort: snapshots that can't be decoded are skipped.

You can use `streamObject` to update your UI incrementally as partial JSON arrives (e.g., show progress or render partial sections).
...
let arrayStream = try streamObjectArray(
  model: openai("gpt-4o"),
  schema: Hero.self,
...
let enumResult = try await generateObjectEnum(
  model: openai("gpt-4o"),
  values: genres,
...
```swift
let noSchema = try await generateObjectNoSchema(
  model: openai("gpt-4o"),
  prompt: "Generate a lasagna recipe."
)
...
You can generate structured data with `generateText` and `streamText` by using the `experimentalOutput` setting.
