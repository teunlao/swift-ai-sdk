...
The mapping from Swift schemas (for example using `schema: MyCodableType.self` or falling back to `FlexibleSchema<MyType>.jsonSchema(...)`) to the JSON that an LLM consumes is not always straightforward, since the mapping is not one-to-one.
