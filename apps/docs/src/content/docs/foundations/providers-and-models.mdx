...
The Swift port currently ships with **35 provider modules** (one SwiftPM product per provider). Each module exports a lazily configured global shortcut plus a factory for advanced configuration, matching the TypeScript behavior. See [Providers](/providers/overview) for the full, up-to-date list.
