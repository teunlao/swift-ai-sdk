...
- [OpenAI Provider](/providers/ai-sdk-providers/openai) — `import OpenAIProvider`, then call `openai("gpt-4o")` or `createOpenAIProvider(settings:)`.
- [Anthropic Provider](/providers/ai-sdk-providers/anthropic) — `import AnthropicProvider`, use `anthropic("claude-3-5-sonnet")` or `createAnthropicProvider(settings:)`.
- [Google Generative AI Provider](/providers/ai-sdk-providers/google-generative-ai) — `import GoogleProvider`, use `google("gemini-1.5-pro")` or `createGoogleGenerativeAI(settings:)`.
- [Groq Provider](/providers/ai-sdk-providers/groq) — `import GroqProvider`, use `groq("llama-3.1-8b-instant")` or `createGroqProvider(settings:)`.
- [OpenAI Compatible Provider](/providers/openai-compatible-providers) — `import OpenAICompatibleProvider`, call `createOpenAICompatibleProvider(settings:)` for any OpenAI-compatible gateway.
