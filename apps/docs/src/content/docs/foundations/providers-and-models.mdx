...
Each shortcut performs lazy API-key loading (via environment variables such as `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `GROQ_API_KEY`) and returns a ready-to-use model for the higher-level Swift AI SDK APIs. OpenAI uses the current Provider V4 facade by default; legacy provider modules may still expose V3-backed models that are adapted by the high-level APIs.
