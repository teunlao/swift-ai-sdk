...
> Note: Swift has no Zod. Most of the time you can lean on `tool(inputSchema: YourCodable.self)`; when you need custom validators, fall back to `AISDKProviderUtils` helpers such as `jsonSchema`/`FlexibleSchema` or `Schema.codable`.
...
Call `.eraseToTool()` when you want to place the typed tool into a `[String: Tool]` dictionary for `generateText`/`streamText`.
...
let result = try await generateText(
  model: openai("gpt-4o"),
  tools: ["getWeather": getWeather.eraseToTool()],
...
let stream = try streamText(
  ...
  tools: [
    ...
    "countToThree": countToThree.eraseToTool()
  ],
