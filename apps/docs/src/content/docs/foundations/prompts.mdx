...
> Warning: Only a few providers and models currently support file parts: [Google Generative AI](/providers/google-generative-ai), [OpenAI](/providers/openai) (for `wav` and `mp3` audio with `gpt-4o-audio-preview`), [Anthropic](/providers/anthropic), [OpenAI](/providers/openai) (for `pdf`).
