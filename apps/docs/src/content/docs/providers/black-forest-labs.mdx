...
let result = try await generateImage(
  ...
  prompt: "A baby elephant with a shirt that has the logo from the input image.",
  files: [
    .url(
...
let result = try await generateImage(
  ...
  files: [
    .url(url: "https://example.com/style-reference.jpg", providerOptions: nil),
    .url(url: "https://example.com/subject-reference.jpg", providerOptions: nil)
  ]
)
```
...
The `flux-pro-1.0-fill` model supports inpainting, which allows you to fill masked regions of an image with new content. Pass the source image via `files:` and a mask image via `mask:`:
...
let result = try await generateImage(
  ...
  files: [
    .url(url: "https://example.com/source-image.jpg", providerOptions: nil)
  ],
  mask: .url(url: "https://example.com/mask-image.png", providerOptions: nil)
)
```
...
> Note: To pass reference images for editing, use `files:` and `mask:` instead
> of provider options. Black Forest Labs supports up to 10 input images.
