...
> Note: In Swift, pass reference images via `prompt: .imageEditing(images:text:mask:)`
> (type `GenerateImagePrompt`). This maps to the provider `files` / `mask` fields.
...
let result = try await generateImage(
  model: togetherai.image("black-forest-labs/FLUX.1-kontext-pro"),
  prompt: .imageEditing(
    images: [.string("https://example.com/photo.png")],
...
> images in `prompt: .imageEditing(images: ...)` are ignored.
