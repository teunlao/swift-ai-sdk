...
// Package.swift
dependencies: [
    .package(url: "https://github.com/teunlao/swift-ai-sdk", from: "0.19.0")
],
targets: [
    .target(
        name: "YourTarget",
...
You can import the default Provider V4 instance `openai` from `OpenAIProvider`:
...
If you need a customized setup, use `createOpenAI(settings:)` with your settings:

```swift
import OpenAIProvider

let provider = try createOpenAI(
...
let model = try provider("gpt-4o")
```

The older `createOpenAIProvider(settings:)` factory remains available for
legacy V3 model integrations.
...
- **baseURL** _string_
  ...
  The default model factory, `openai("model-id")`, uses the Responses API. If
  your custom base URL only supports the Chat Completions API, create chat
  models with `openai.chat("model-id")` instead or use the OpenAI-compatible
  provider.
...
- **fetch** _FetchFunction_
  ...
  or to provide a custom fetch implementation for testing.
...
The default OpenAI model factory uses the Responses API. If you want to
explicitly select a specific model API, use `.responses`, `.chat`, or
`.completion`.
...
let result = try await generateText(
  model: try openai("gpt-5"), // or openai.responses("gpt-5")
  providerOptions: [
    "openai": [
...
- **reasoningMode** _'standard' | 'pro'_
  Controls how much model work GPT-5.6 performs before returning a final answer. Use `pro` when quality matters more than latency and token usage.

- **reasoningContext** _'auto' | 'current_turn' | 'all_turns'_
  Controls which available reasoning items GPT-5.6 can reuse. The effective context is returned as `providerMetadata.openai.reasoningContext`.
...
- **include** _Array&lt;string&gt;_
  ...
  `['web_search_call.results']` for including web search results.
...
- **promptCacheOptions** _object_
  Configures prompt caching for GPT-5.6 and later models. `mode` can be `implicit` or `explicit`, and `ttl` currently supports `30m`. Explicit mode caches only content blocks marked with `promptCacheBreakpoint`.

- **promptCacheRetention** _'in_memory' | '24h'_
  Legacy cache retention for models before GPT-5.6. For GPT-5.6 and later models, use `promptCacheOptions.ttl`.
...
- **serviceTier** _(optional)_
  Service tier returned by the API.

- **reasoningContext** _(optional)_
  Effective persisted-reasoning context returned by GPT-5.6.

Cache reads and writes are reported through `result.usage.inputTokens.cacheRead` and `result.usage.inputTokens.cacheWrite`; reasoning tokens are reported through `result.usage.outputTokens.reasoning` on the provider V4 result.
...
The OpenAI responses API supports the local shell tool for Codex models through the `openai.tools.localShell` tool.
Local shell lets the model propose shell commands that your integration executes locally and returns the output.
  ...
  tools: [
    "local_shell": openai.tools.localShell(.init(
      execute: { input, _ in
        ...
        // parsed.action.command is [String]
        // ... run parsed.action.command ...
...
<Aside type="note">
  The tool must be named `local_shell`. This name is required by OpenAI's API specification and cannot be customized.
</Aside>

<Aside type="note">
  Codex-oriented Responses models such as `gpt-5.3-codex` can emit
  `providerMetadata.openai.phase` with values like `commentary` and
  `final_answer` on assistant text parts. Preserve that metadata if you send
  the conversation back to the OpenAI Responses API for a follow-up turn.
</Aside>
...
<Aside type="caution">
  Running arbitrary shell commands can be dangerous. Always sandbox execution or enforce strict allow-/deny-lists before forwarding a command to the system shell.
</Aside>
...
let result = try await generateText(
  ...
  tools: [
    "shell": openai.tools.shell(.init(
      execute: { input, _ in
        let parsed = try await validateTypes(
          ValidateTypesOptions(value: input, schema: openaiShellInputSchema)
        )

        // parsed.action.commands is [String]
        // ... run parsed.action.commands ...
...
The OpenAI Responses API supports the apply patch tool through the `openai.tools.applyPatch` tool.
The model emits structured patch operations that your application applies and reports back on.
...
let result = try await generateText(
  ...
  tools: [
    "apply_patch": openai.tools.applyPatch(.init(
      execute: { input, _ in
        let parsed = try await validateTypes(
          ValidateTypesOptions(value: input, schema: openaiApplyPatchInputSchema)
        )

        // parsed.callId, parsed.operation
        // ... apply patch ...
...
The OpenAI Responses API supports provider-executed MCP (Model Context Protocol) tools via the `openai.tools.mcp` tool.
...
let result = try streamText(
  ...
  experimentalApprove: { _ in
    // Approve (or deny) provider-executed MCP calls when OpenAI requests approval.
    .approve
  }
)
...
When an MCP tool is called, the tool name is emitted as `mcp.<toolName>` (for example: `mcp.shorten`).

<Aside type="note">
  MCP tool results are provider-executed and are surfaced as tool calls/results with a JSON output payload:
  <code>{ '{' } type: "call", serverLabel, name, arguments, output?, error? { '}' }</code>.
</Aside>
...
The OpenAI Responses API supports structured outputs. You can enforce structured outputs using `generateObject` or `streamObject`, which expose a `schema` option. Additionally, you can pass a Zod or JSON Schema object to the `experimentalOutput` option when using `generateText` or `streamText`.
...
- **promptCacheOptions** _object_
  ...
  Configures prompt caching for GPT-5.6 and later models. `mode` can be `implicit` or `explicit`, and `ttl` currently supports `30m`.
...
- You can control how system messages are handled by providerOptions `systemMessageMode`:
  ...
  - `developer`: treat the system message as a developer message (default for reasoning models).
  - `system`: keep the system message as a system-level instruction.
  - `remove`: omit the system message from the request.
...
GPT-5.6 also supports explicit cache control. Set `promptCacheOptions.mode` to `explicit` and mark each cache boundary with `promptCacheBreakpoint`:
...
let result = try await openai.responses("gpt-5.6-sol").doGenerate(
  options: .init(
    prompt: [
      .system(content: "Stable instructions", providerOptions: breakpoint),
      .user(
        content: [.text(.init(text: "Reusable context", providerOptions: breakpoint))],
        providerOptions: nil
      )
    ],
    providerOptions: [
...
print(result.usage.inputTokens.cacheRead as Any)
print(result.usage.inputTokens.cacheWrite as Any)
```
