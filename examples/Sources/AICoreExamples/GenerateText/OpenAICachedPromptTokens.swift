...
struct GenerateTextOpenAICachedPromptTokensExample: Example {
  static let name = "generate-text/openai-cached-prompt-tokens"
  static let description = "Demonstrates prompt caching effects when reusing a long input." 
  ...
  static func run() async throws {
    do {
      ...
      Logger.info("Caching behavior may require a few runs; retry if you miss a cache hit.")

      let (firstResult, firstDuration) = try await Helpers.measure {
        try await generateText(
          model: openai("gpt-4o-mini"),
          messages: [
            .user(
              UserModelMessage(
                content: .text("What book is the following text from?: <text>\(longPrompt)</text>")
              )
            )
          ],
      ...
      Logger.section("First Pass")
      Logger.info("Text: \(firstResult.text)")
      Logger.info("Time: \(Helpers.formatDuration(firstDuration))")
      Logger.section("First Pass Usage")
      Helpers.printJSON(firstResult.usage)
      if let metadata = firstResult.providerMetadata {
        Logger.section("First Pass Provider Metadata")
        Helpers.printJSON(metadata)
      }

      try await Task.sleep(nanoseconds: 1_000_000_000)

      let (secondResult, secondDuration) = try await Helpers.measure {
        try await generateText(
          model: openai("gpt-4o-mini"),
          messages: [
            .user(
      ...
      Logger.section("Second Pass")
      Logger.info("Text: \(secondResult.text)")
      Logger.info("Time: \(Helpers.formatDuration(secondDuration))")
      Logger.section("Second Pass Usage")
      Helpers.printJSON(secondResult.usage)
      if let metadata = secondResult.providerMetadata {
        Logger.section("Second Pass Provider Metadata")
        Helpers.printJSON(metadata)
      }
    } catch {
