...
export class OrchestratorDB {
  ...
  constructor(options: OrchestratorDBOptions = {}) {
    ...
    const defaultDbPath = join(dirname(this.schemaPath), "../orchestrator.db");
    const dbPath = options.dbPath ?? envPath ?? defaultDbPath;
  ...
  addLog(log: Omit<AgentLog, "id">): Required<AgentLog> {
    const stmt = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `);
    const result = stmt.run(log.agent_id, log.timestamp, log.event_type, log.content);
    const stored: Required<AgentLog> = {
      id: Number(result.lastInsertRowid),
      ...log,
    };
    this.events.publish({ type: "log-added", agentId: log.agent_id, log: stored });
    return stored;
  }
