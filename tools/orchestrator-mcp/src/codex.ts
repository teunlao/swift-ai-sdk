...
`;
	...
	const codexProcess = spawn("bash", [scriptFile], {
		stdio: "ignore",
		cwd: cwd,
		detached: true,
	});
 ...
 */
export interface ContinueCodexOptions {
	cwd?: string;
	model?: string;
	reasoningEffort?: "low" | "medium" | "high";
}
...
export async function continueCodexAgent(
	...
	try {
		...
		if (options.cwd) {
			request.params.arguments.cwd = options.cwd;
		}

		// Add optional model parameter
		if (options.model) {
			request.params.arguments.model = options.model;
		}
