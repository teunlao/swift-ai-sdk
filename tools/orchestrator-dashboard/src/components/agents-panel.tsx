...
function AgentRow({ agent }: { agent: AgentSummary }) {
  return (
    <tr className="border-b border-white/5">
      ...
      <td className="py-2 text-sm text-neutral-400">{formatDate(agent.lastActivity)}</td>
      <td className="py-2 text-sm text-neutral-300">
        {agent.validation ? <StatusBadge status={agent.validation.status} /> : "–"}
      </td>
    </tr>
  );
}
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            Track executors and validators, their status, and validation progress.
          </p>
        </div>
            ...
            {STATUS_FILTERS.map((item) => (
              <button
                ...
                className={
                  ...
                  (status === item.value ? "bg-white/10 text-white" : "hover:bg-white/5")
                }
              >
                {item.label}
              </button>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={8} className="px-4 py-6 text-center text-sm text-neutral-400">
                  {loading ? "Loading agents..." : "No agents match the selected filter."}
                </td>
              </tr>
            )}
          </tbody>
